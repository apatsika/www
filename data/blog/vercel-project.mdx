...
Vercel provides a domain that points to the `vercel.app` by default. I configured a custom domain for my website for a domain I purchased through Godaddy. The domain panel makes it easy to add a new domain by typing the URL of the domain name. I was given the directions to update the DNS records in Godaddy. The domain points to my production environment in Vercel.

![Giscus Environment Variables](/static/images/domain.png)

## Security

Security is not an afterthought in Vercel. The free tier Hobby account offers Log and Source protection as well as Git Fork Protection. Log and Source protection hides the `_src` and `_logs` path by default. These paths are only accessible to me or to members that I have added as users to my team in the Vercel panel. Git Fork protection prevents the leaking of sensitive project information by requiring authorization to deploy a pull request. I will not touch on the other features since they require an upgrade form the Hobby account. 

Additionally, all network traffic is encrypted by default at no additional cost. Vercel issues free SSL certificates using a non profit plaftorm called **Let's Encrypt**. You can find more on this topic here: https://vercel.com/blog/automatic-ssl-with-vercel-lets-encrypt
